...
dac_continuous_handle_t sig_chan;
#else
...
void bt_i2s_driver_install(void)
{
    ...
    // use I2S channel 1 & external DAC for audio output
    ...
    // use continuous dac as vibration data output, do not enable it here
    // dac_continuous_config_t cont_cfg = {
    //     .chan_mask = DAC_CHANNEL_MASK_ALL,
    //     .desc_num = 8,
    //     .buf_size = 2048,
    ...
    // };
    /* Allocate continuous channels */
    // ESP_ERROR_CHECK(dac_continuous_new_channels(&cont_cfg, &sig_chan));
    /* Enable the continuous channels */
    // ESP_ERROR_CHECK(dac_continuous_enable(sig_chan));
#endif
}
...
void bt_i2s_driver_uninstall(void)
{
    ...
    ESP_ERROR_CHECK(dac_continuous_disable(sig_chan));
    ESP_ERROR_CHECK(dac_continuous_del_channels(sig_chan));
#endif
}
...
static void bt_av_hdl_a2d_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_A2D_AUDIO_CFG_EVT: {
        ...
        #endif
            // dac_continuous_disable(sig_chan);
            // dac_continuous_del_channels(sig_chan);
            dac_continuous_config_t cont_cfg = {
                .chan_mask = DAC_CHANNEL_MASK_ALL,
                .desc_num = 8,
                .buf_size = 2048,
                .freq_hz = sample_rate/4,
                .offset = 0,
                .clk_src = DAC_DIGI_CLK_SRC_APLL, //DAC_DIGI_CLK_SRC_DEFAULT,   // Using APLL as clock source to get a wider frequency range
                .chan_mode = DAC_CHANNEL_MODE_ALTER,
            };
            /* Allocate continuous channels */
            dac_continuous_new_channels(&cont_cfg, &sig_chan);
            /* Enable the continuous channels */
            dac_continuous_enable(sig_chan);
