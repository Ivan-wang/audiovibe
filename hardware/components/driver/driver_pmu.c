...
__attribute__((weak)) __attribute__((section("ram_code"))) void charge_isr_ram(uint8_t type)
{
    if(type == 2)
    {
        pmu_disable_isr(PMU_ISR_BIT_BAT);
        pmu_enable_isr(PMU_ISR_BIT_ACOFF);
    }
    else if(type == 1)
    {
        pmu_disable_isr(PMU_ISR_BIT_BAT);
        pmu_enable_isr(PMU_ISR_BIT_ACOK);
    ...
    {
        pmu_disable_isr(PMU_ISR_BIT_ACOK);
        pmu_enable_isr(PMU_ISR_BIT_ACOFF|PMU_ISR_BIT_BAT);
        log_printf("charge in\r\n");
		system_sleep_disable();
		pmu_set_led2_value(1);			//charging led on
    }
}
...
{
    log_printf("gpio new value = 0x%08x.\r\n", ool_read32(PMU_REG_GPIOA_V));
}
...
__attribute__((section("ram_code"))) void pmu_isr_ram_C(unsigned int* hardfault_args)
{
    ...
    {
        //log_printf("RTC ALARM A interrupt.\r\n");
    ...
    {
        //log_printf("%04x", ool_read16(PMU_REG_QDEC_CNTA_VALUE));
