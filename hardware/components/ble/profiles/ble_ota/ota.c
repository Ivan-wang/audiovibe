...
void __attribute__((weak)) ota_stop(ota_warning_type evt_id)
{
    ...
    {
        case OTA_TIMOUT:
            log_printf("OTA_TIMOUT\r\n");
            break;
        case OTA_ADDR_ERROR:
            log_printf("OTA_ADDR_ERROR\r\n");
            break;
        case OTA_CHECK_FAIL:
            log_printf("OTA_CHECK_FAIL\r\n");
            break;
    };
}
void os_timer_ota_cb(void *arg)
...
//write user data.
        ...
        case OTA_CMD_REBOOT:
            if(first_pkt.buf != NULL)
            {
                ...
                flash_protect_enable(0);
#endif	
				log_printf("crc32 check success\r\n");
                app_otas_save_data(new_bin_base,first_pkt.buf,256);
#ifdef OTA_CRC_CHECK				
                }
                else{
                    log_printf("crc32 check fail\r\n\r\n");
                    os_free(req);
...
{   
    ...
    uint8_t ret = 0;
    log_printf("crc32_check_addr: 0x%x\r\n",crc32_check_addr);
